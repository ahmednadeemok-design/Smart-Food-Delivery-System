...
export default function MenuManagement() {
  ...
  const submitItem = async ({ restaurantId, payload }) => {
    ...
    try {
      await addFoodItem(targetRestaurantId, payload);
  ...
  const toggleAvailability = async (item) => {
    try {
      const res = await updateFoodItem(selectedRestaurantId, item._id, { isAvailable: !item.isAvailable });
  ...
  const removeItem = async (item) => {
    try {
      await deleteFoodItem(selectedRestaurantId, item._id);
